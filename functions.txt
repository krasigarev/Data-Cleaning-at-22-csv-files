df = pd.read_csv('dataverse_files/1993.csv.bz2', engine='pyarrow')
df
...
def drop_columns(df):
    columns = ['TailNum', 'SecurityDelay', "TaxiIn", 'TaxiOut', 'LateAircraftDelay', 'CancellationCode', 'CarrierDelay', 'NASDelay', 'WeatherDelay', 'AirTime']
...
def save_to_csv(df):
    df.to_csv('df_1992.csv', index=False, compression='gzip')
